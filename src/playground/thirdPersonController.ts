...
import { AbstractMesh, ActionManager, ArcRotateCamera, AssetContainer, Engine, ExecuteCodeAction, IPhysicsCollisionEvent, ISceneLoaderProgressEvent, Mesh, MeshBuilder, Nullable, PhysicsAggregate, PhysicsEventType, PhysicsMotionType, PhysicsRaycastResult, PhysicsShapeType, Quaternion, Ray, RayHelper, Scalar, Scene, SceneLoader, Sound, Vector3 } from '@babylonjs/core';
...
export class ThirdPersonController {
    ...
    // 地板物理光线拾取
    private physEngine: Nullable<IPhysicsEngine>;
    ...
    private deltaTime = '0';
    private wallkingSound!: Sound;

    // 实时足部光线投射 状
    private footRaycast = new PhysicsRaycastResult();
    private jumpState = {
    ...
    // 楼梯光线
    private staircaseRay!: Ray;
    private staircaseRaycast = new PhysicsRaycastResult();
    ...
    private async initGenerate() {
        this.meshContent = await this.loadAsset('model/', 'x-bot.glb');
        const [mesheRoot] = this.meshContent.meshes;
        mesheRoot.receiveShadows = true;
        this.meshContent.addAllToScene();

        this.boxHelper = MeshBuilder.CreateBox('lbl', { height: 3.2 }, this.scene);
        ...
        mesheRoot.scaling = new Vector3(2, 2, 2);
        player.position.y = 5;
        player.position.z = -5;
        ...
        // @ts-ignore
        this.staircaseRay = new Ray();
        const rayHelper = new RayHelper(this.staircaseRay);
        rayHelper.attachToMesh(
            mesheRoot,
            new Vector3(0, -1, 0),
            new Vector3(0, 0.4, 0.36),
            0.23
        );
    ...
    private onBeforeAnimations = () => {
        ...
        if (this.curAnimParam.weight < 1) {
            this.curAnimParam.weight = Scalar.Clamp(this.curAnimParam.weight + 0.05, 0, 1);
        ...
        if (this.oldAnimParam.weight > 0) {
            this.oldAnimParam.weight = Scalar.Clamp(this.oldAnimParam.weight - 0.05, 0, 1);
    ...
    private updateState = (): void => {
        ...
        const start = new Vector3(x, res, z);
        const end = new Vector3(x, res - 0.8, z);
        (this.physEngine as any).raycastToRef(start, end, this.footRaycast);

        const s_start = this.staircaseRay.origin.add(new Vector3(0, 0, 0.03));
        const s_end = s_start.add(new Vector3(0, -0.68, 0));
